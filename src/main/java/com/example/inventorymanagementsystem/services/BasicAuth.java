...
public class BasicAuth {
    ...
    public static boolean isAuthentication(@Context ContainerRequestContext containerRequestContext, List<String> userRoles) {
        ...
        if (authHeader != null && authHeader.startsWith(AUTHORIZATION_HEADER_PREFIX)) {
            ...
            isUser = user.isUser(username, password);

            if (isUser != null) {
                if(userRoles.contains(isUser.getUserRole())){
    ...
    private static String getLogMessage(ContainerRequestContext requestContext, String message) {
